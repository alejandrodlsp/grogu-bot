...
class Fun(commands.Cog):
    ...
    @cooldown(CD_RATE, CR_DURATION, BucketType.guild)
    @commands.command(name="pixelate", aliases=get_aliases("pixelate"))
    async def pixelate_command(self, ctx, member: discord.Member = None):
        if not member:
            member = ctx.author
        await request_canvas_image(ctx, "https://some-random-api.ml/canvas/pixelate", member)
